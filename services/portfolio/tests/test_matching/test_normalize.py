...
class TestInvisibleCharacters:
    ...
    def test_smart_quotes_survive(self):
        ...
        assert normalize_posting_text("“DevOps”") == "“DevOps”"


class TestLayout:
    def test_bullet_glyphs_stripped(self):
        assert normalize_posting_text("• Python\n• Django") == "Python\nDjango"

    def test_blank_runs_collapse_to_one_break(self):
        assert (
            normalize_posting_text("Requirements\n\n\n\nPython")
            == "Requirements\n\nPython"
        )
...
class TestProperties:
    ...
    def test_is_idempotent(self, raw):
        ...
        assert normalize_posting_text(once) == once

    @given(raw=st.text(max_size=300))
    def test_never_raises_and_returns_str(self, raw):
        assert isinstance(normalize_posting_text(raw), str)
