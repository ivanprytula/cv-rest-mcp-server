...
Note: `set dotenv-load` injects `.env` into every just recipe. A manually started uvicorn does NOT read `.env` — source it (`set -a; source .env; set +a`) first.
