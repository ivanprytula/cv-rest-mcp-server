...
from app.failban import ViolationTracker
from app.guard_middleware import GuardMiddleware
from app.ip_lists import ip_in_networks, load_ip_list, parse_ip_list
from app.service_hours import is_within_service_hours, parse_service_days
...
async def test_guard_closed_hours_denies_503(monkeypatch):
    ...
    monkeypatch.setattr("app.guard_middleware.is_within_service_hours", lambda: False)
    sent = await _run_guard(guard, _scope())
    status = next(m for m in sent if m["type"] == "http.response.start")
...
async def test_guard_open_hours_pass_through(monkeypatch):
    ...
    monkeypatch.setattr("app.guard_middleware.is_within_service_hours", lambda: True)
    sent = await _run_guard(guard, _scope())
    assert {"type": "passthrough"} in sent
